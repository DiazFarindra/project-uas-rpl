...
<div class="card bg-white shadow-md w-full">
    ...
    <div class="card-body p-6">
        ...
        <a href="{{ route('admin.informations.create') }}" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg inline-flex items-center">
            Tambah
        </a>
            ...
            <table class=" w-full border-collapse border border-gray-200 shadow-sm rounded-md">
                <thead class="bg-gray-100">
                    <tr>
                        ...
                        <th class="border border-gray-300 px-4 py-2 text-left font-semibold text-gray-600">khusus untuk (optional)</th>
                        <th class="border border-gray-300 px-4 py-2 text-left font-semibold text-gray-600">judul</th>
                        <th class="border border-gray-300 px-4 py-2 text-left font-semibold text-gray-600">content</th>
                ...
                <tbody>
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="border border-gray-300 px-4 py-2">{{ $information->user?->name ?? 'semua' }}</td>
                        <td class="border border-gray-300 px-4 py-2">{{ $information->title }}</td>
                        <td class="border border-gray-300 px-4 py-2">{{ $information->content }}</td>
                        <td class="border border-gray-300 px-4 py-2 flex justify-center space-x-2">
                            <a href="{{ route('admin.informations.edit', $information->id) }}" class="bg-blue-400 hover:bg-blue-500 text-white px-3 py-1 rounded-md flex items-center">
                                edit
                            </a>
                            <form action="{{ route('admin.informations.destroy', $information->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?');">
                                @csrf
                                @method('delete')
