...
<div class="card bg-white w-full">
    ...
    <div class="card-body p-6">
        ...
        <form action="{{ route('admin.accounts.store') }}" method="post" class="grid grid-cols-1 gap-4">
            @csrf
            ...
            <div class="flex flex-col space-y-2">
                ...
                <input type="number" name="nim" id="nim" class="form-input" value="{{ old('nim') }}">
                @error('nim')
                <p class="text-red-500 text-sm">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex flex-col space-y-2">
                ...
                <select name="type" id="type" class="form-select">
                    ...
                    <option {{ old('type') === \App\Enums\UserType::ADMIN->value ? 'selected' : '' }} value="{{ \App\Enums\UserType::ADMIN->value }}">Admin</option>
                    <option {{ old('type') === \App\Enums\UserType::STUDENT->value ? 'selected' : '' }} value="{{ \App\Enums\UserType::STUDENT->value }}">Mahasiswa</option>
                    <option {{ old('type') === \App\Enums\UserType::TEACHER->value ? 'selected' : '' }} value="{{ \App\Enums\UserType::TEACHER->value }}">Dosen/Pembimbing</option>
                </select>
                @error('type')
                <p class="text-red-500 text-sm">{{ $message }}</p>
                @enderror
