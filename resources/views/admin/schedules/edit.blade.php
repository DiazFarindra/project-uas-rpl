...
<div class="card bg-white w-full">
    ...
    <div class="card-body p-6">
        ...
        <form action="{{ route('admin.schedules.update', $schedule->id) }}" method="post" class="grid grid-cols-1 gap-4">
            @csrf
            @method('put')
            ...
            <div class="flex flex-col space-y-2">
                ...
                <select name="teacher_id" id="teacher_id" class="border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:border-blue-500">
                    ...
                    <option {{ $schedule->teacher_id == $teacher->id ? 'selected' : '' }} value="{{ $teacher->id }}">{{ $teacher->name }}</option>
                    @empty
            ...
            <div class="flex flex-col space-y-2">
                ...
                <select name="student_id" id="student_id" class="border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:border-blue-500">
                    ...
                    <option {{ $schedule->student_id == $student->id ? 'selected' : '' }} value="{{ $student->id }}">{{ $student->name }}</option>
                    @empty
                    <option selected disabled>Tidak ada data</option>
                    @endforelse
                </select>
            ...
            <div class="flex flex-col space-y-2">
                ...
                <input type="datetime-local" name="date_time" id="date_time" class="form-input" value="{{ $schedule->date_time->format('Y-m-d\TH:i') }}">
                @error('date_time')
                <p class="text-red-500 text-sm">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex flex-col space-y-2">
                ...
                <input type="text" name="title" id="title" class="form-input" value="{{ $schedule->title }}">
                @error('title')
                <p class="text-red-500 text-sm">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex flex-col space-y-2">
                ...
                <textarea name="description" id="description" class="form-textarea">{{ $schedule->description }}</textarea>
                @error('description')
                <p class="text-red-500 text-sm">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex flex-col space-y-2">
                ...
                <select name="status" id="status" class="border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:border-blue-500">
                    <option {{ $schedule->status == 'pending' ? 'selected' : '' }} value="pending">Pending</option>
                    <option {{ $schedule->status == 'approved' ? 'selected' : '' }} value="approved">Approved</option>
                    <option {{ $schedule->status == 'overdue' ? 'selected' : '' }} value="overdue">Overdue</option>
                </select>
                @error('status')
                <p class="text-red-500 text-sm">{{ $message }}</p>
                @enderror
