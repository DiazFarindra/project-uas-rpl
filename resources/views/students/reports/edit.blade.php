...
<div class="card bg-white w-full">
    ...
    <div class="card-body p-6">
        ...
        <form action="{{ route('student.reports.update', $report->id) }}" method="post" class="grid grid-cols-1 gap-4">
            @csrf
            @method('put')

            <div class="flex flex-col space-y-2">
                <label for="title" class="text-gray-600">Judul</label>
                <input type="text" name="title" id="title" class="form-input" value="{{ $report->title ?? old('title') }}">
                @error('title')
            ...
            <div class="flex flex-col space-y-2">
                ...
                <textarea name="content" id="content" class="form-textarea">{{ $report->content ?? old('content') }}</textarea>
                @error('content')
                <p class="text-red-500 text-sm">{{ $message }}</p>
                @enderror
            </div>
